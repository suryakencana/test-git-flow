...
class Manga(Slugger, SurrogatePK, Base):
    ...
    # sp, kk,
    type = Column(CHAR(2))
    # bt(batoto), ed(mangaeden)
    # group scanlation/sumber
    ...
    @classmethod
    def get_prev(cls, manga, cur_chapter):
        return Chapter.query \
            .order_by(desc(Chapter.sortorder)) \
            .filter_by(tb_manga_id=manga.id) \
            .filter(Chapter.sortorder < cur_chapter.sortorder) \
            .first()

    @classmethod
    def get_next(cls, manga, cur_chapter):
        return Chapter.query \
            .order_by(desc(Chapter.sortorder)) \
            .filter_by(tb_manga_id=manga.id) \
            .filter(Chapter.sortorder > cur_chapter.sortorder) \
